...
# High level overview
- The tool will trigger folders/ single files transfers via Bittorrent's protocol. It will act as a move command, deleting the source file from the sender after it has been downloaded successfully by the receiver.

- It can work in "single sender -> single receiver" or "multi senders -> single receiver" configurations.
